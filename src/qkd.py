...
def bb84_key_generation(n_bits: int = 256,
    ...
    alice_bases = np.fromiter((rng.randint(0, 1) for _ in range(n_bits)), dtype=np.int8)
    bob_bases   = np.fromiter((rng.randint(0, 1) for _ in range(n_bits)), dtype=np.int8)

    backend = _sim_for_noise(noise_level)

    try:
        max_qubits = int(getattr(getattr(backend, "target", None), "num_qubits", None))
    ...
    qber = errors / len(sample_idx) if len(sample_idx) > 0 else 1.0

    if qber > qber_threshold:
        if raise_on_high_qber:
            raise ValueError(f"High QBER: {qber:.2%}")
        return None, qber
